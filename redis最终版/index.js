...
var logwatch_button = true;//循环拦截模块默认开启，可以选择在前端页面中设置成关闭
var messages = [];
var baozhangs = [];
var pers = [];
var snaps = [];
...
io.sockets.on('connection',function (socket) {
    ...
    //     eventEmitter.emit('persistence_button',persistence_button);
    //     // $scope.$apply(function () {
    //     //     $scope.persistence_button = persistence_button;
    //     // });
    // });
        ...
        socket.on('addlogwatch_button',function (logwatch_button) {
            eventEmitter.emit('logwatch_button',logwatch_button);
        });
    ...
    eventEmitter.on('addper',function (per) {
        pers.push(per);
        io.sockets.emit('newper',per);
    });
...
//持久化检查模块
//var persistence_button = true;//默认要检查，可以选择在前端页面中设置成关闭
// eventEmitter.on('persistence_button',function (persistence_button) {
//
// });
if(persistence_button == true) {
    setTimeout(function () {
        eventEmitter.emit('addper','开启');
    },2000);
    //console.log('per');
...
if(logwatch_button) {
    ...
    setTimeout(function () {
        eventEmitter.emit('addxhreason','尚未循环中断');
    },2000);
    setTimeout(function () {
        eventEmitter.emit('addstatusxg','尚未循环中断');
    },2000);
}
else {
    setTimeout(function () {
        eventEmitter.emit('addxhreason','循环拦截模块未开启');
    },2000);
    setTimeout(function () {
        eventEmitter.emit('addstatusxg','循环拦截模块未开启');
    },2000);
}
eventEmitter.on('logwatch_button',function (logwatch_button) {
    if(logwatch_button) {
        fs.watchFile(fileName,function(curr, prev){
            console.log('interrupt:the current mtime is: ' + curr.mtime);
            console.log('interrupt:the previous mtime was: ' + prev.mtime);
